...
public:
  SpriteHolder(size_t width, size_t height, size_t channels, std::vector<uint8_t> image_pixels)
      : width(width), height(height), channels(channels), image_pixels(std::move(image_pixels)) {
  ...
  const uint8_t getPixelValue(size_t x, size_t y, size_t channel) const {
    assert(x < width && y < height && channel < channels && "Pixel coordinates out of bounds");
    return image_pixels[(y * width + x) * channels + channel];
  };

  const uint8_t getPixelValue(size_t cell_index, size_t channel) const {
    assert(cell_index < num_pixels && channel < channels && "Pixel coordinates out of bounds");
    return image_pixels[cell_index * channels + channel];
  };
};