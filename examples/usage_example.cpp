...
int main() {
	...
	OverlappingPatterns overlapping_patterns(
			sprite, N, BoundaryCondition::NONE, SpriteTransforms::ALL_TRANSFORMS);
	...
	bool force_boundary_patterns =
			false; // if true, enforce boundaries (e.g. walls) around the image
	...
	for (size_t i = 0; i < num_images; ++i) {
		...
		collapsed_grid = wfc.solve(grid_width, grid_height, start_index, force_boundary_patterns,
				CellSelectionStrategy::SCANLINE, fixed_cells, -1);
