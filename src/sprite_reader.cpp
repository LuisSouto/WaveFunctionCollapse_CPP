...
SpriteHolder SpriteReader::loadFromPng(const char *filename,
  ...
  size_t total_bytes =
      static_cast<size_t>(width) * height * desired_channels * sizeof(uint8_t);
  std::vector<uint8_t> image_pixels_rgb(image_data, image_data + total_bytes);
  stbi_image_free(image_data);

  return SpriteHolder(static_cast<size_t>(width), static_cast<size_t>(height),
                      desired_channels, image_pixels_rgb);
}