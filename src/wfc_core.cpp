...
std::span<const pattern_id_t> WFCCore::solve(size_t output_width, size_t output_height,
		...
		CellSelectionStrategy cell_selection_strategy, BoundaryCondition boundary_condition) {
	bool success = generateCollapsedGrid(output_width, output_height, start_index,
			force_boundary_patterns, cell_selection_strategy, boundary_condition);
	while (!success) {
		std::cerr << "Failed to generate a valid collapsed grid. Retrying..." << std::endl;
...
bool WFCCore::generateCollapsedGrid(size_t output_width, size_t output_height, size_t start_index,
...
void WFCCore::initializeGrid(size_t output_width, size_t output_height) {
	...
	for (size_t y = 0; y < output_height; ++y) {
		for (size_t x = 0; x < output_width; ++x) {
			...
			impossible_grid =
					impossible_grid && (grid[cell_index * num64_blocks + num64_blocks - 1] == 0);
			if (impossible_grid) {
	...
	for (size_t cell_index : temp_buffers.current_cell_wave) {
		...
		for (size_t i = 0; i < NUM_DIRECTIONS_2D; i++) {
			...
			if (i + 1 < NUM_DIRECTIONS_2D) {
				size_t next_idx = neighbour_indexes[cell_index * NUM_DIRECTIONS_2D + i + 1];
